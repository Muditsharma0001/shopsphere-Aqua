...
export default function AdminLogin() {
  ...
  const handleDevBypass = () => {
    ...
    window.location.href = `${apiUrl}/auth/dev-login?role=ADMIN`;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setErrorMsg(null);
    try {
      // Direct local developer bypass on login submit for quick access
      handleDevBypass();
    } catch (err: any) {
        ...
        <div className="w-full max-w-md px-6 py-12 relative z-20">
          <div className="rounded-3xl border border-zinc-900 bg-zinc-950/40 p-8 backdrop-blur-2xl shadow-2xl space-y-6">
            <div className="text-center">
              <span className="text-3xl filter drop-shadow-[0_0_8px_rgba(99,102,241,0.25)]">🛡️</span>
              <h2 className="mt-4 text-xl font-black text-white uppercase tracking-wider">Administrator Sign In</h2>
              <p className="text-[10px] text-zinc-500 uppercase tracking-widest mt-1">ShopSphere Platform Controls</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-[9px] text-zinc-500 uppercase tracking-widest font-semibold block">Admin Email</label>
                <input
                  type="email"
